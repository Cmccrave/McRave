...
namespace BWEB {
    ...
    void Wall::initializePathPoints()
    {
        ...
        /*if (Map::mapBWEM.GetArea(TilePosition(perpLine.first)) == area) {
            pathStart = Map::mapBWEM.GetArea(TilePosition(perpLine.first))->Top();
            pathEnd = Map::mapBWEM.GetArea(TilePosition(perpLine.second))->Top();
        }
        else {
            pathStart = Map::mapBWEM.GetArea(TilePosition(perpLine.second))->Top();
            pathEnd = Map::mapBWEM.GetArea(TilePosition(perpLine.first))->Top();
        }*/

        if (choke == Map::getNaturalChoke()) {
            initialPathStart = TilePosition(Map::getMainChoke()->Center());
    ...
    void Wall::initialize()
    {
        ...
        const auto scoreWall = [&] {
            ...
            const auto score = !openWall ? dist / exp(centroid.getDistance(Position(choke->Center()))) : (pathOut.getDistance()) / log(max(32.0, dist));
            /// -------------
        ...
        function<void(TilePosition)> recursiveCheck = [&](TilePosition start) -> void {
            ...
            for (auto x = start.x - radius; x < start.x + radius; x++) {
                for (auto y = start.y - radius; y < start.y + radius; y++) {
                    ...
                    if (openWall && center.getDistance(base->Center()) > 320.0)
                        continue;
