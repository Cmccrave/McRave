...
namespace McRave::Spy::Protoss {
    ...
    void enemyProtossOpeners(PlayerInfo& player, StrategySpy& theSpy)
    {
        ...
        if (theSpy.build.name == P_2Gate) {
            ...
            if (theSpy.proxy.possible) {
                auto closestGate = Util::getClosestUnit(Terrain::getMainPosition(), PlayerState::Enemy, [&](auto &u) {
                    return u->getType() == Protoss_Gateway;
                });
    ...
    void enemyProtossTransitions(PlayerInfo& player, StrategySpy& theSpy)
    {
        ...
        if (theSpy.build.name == P_2Gate || theSpy.build.name == P_1GateCore) {
            ...
            // Corsair
            if ((theSpy.build.name == P_2Gate && completesBy(1, Protoss_Stargate, Time(6, 30)))
                || (theSpy.build.name == P_1GateCore && completesBy(1, Protoss_Stargate, Time(5, 00)))
                || (theSpy.build.name == P_2Gate && completesBy(1, Protoss_Scout, Time(7, 00)))
                || (theSpy.build.name == P_2Gate && completesBy(1, Protoss_Corsair, Time(7, 00)))
                || (theSpy.build.name == P_2Gate && arrivesBy(1, Protoss_Corsair, Time(7, 30)))
                || (theSpy.build.name == P_1GateCore && completesBy(1, Protoss_Scout, Time(5, 15)))
                || (theSpy.build.name == P_1GateCore && completesBy(1, Protoss_Corsair, Time(5, 15)))
                || (theSpy.build.name == P_1GateCore && arrivesBy(1, Protoss_Corsair, Time(5, 45))))
            ...
            if (Players::ZvP()) {
                ...
                if (Players::getVisibleCount(PlayerState::Enemy, Protoss_Corsair) == 0 && Players::getVisibleCount(PlayerState::Enemy, Protoss_Stargate) == 0) {
                    if ((!theSpy.expand.possible && Players::getTotalCount(PlayerState::Enemy, Protoss_Dragoon) >= 4 && Util::getTime() < Time(6, 00))
                        ...
                        || (completesBy(1, Singularity_Charge, Time(6, 00)))
                        || (Spy::enemyTurtle() && completesBy(1, Singularity_Charge, Time(6, 30)))
        ...
        if (Players::ZvP() && theSpy.build.name == P_FFE) {
            ...
            // 5GateGoon
            if ((Players::getTotalCount(PlayerState::Enemy, Protoss_Dragoon) >= 4 && Players::getTotalCount(PlayerState::Enemy, Protoss_Corsair) == 0 && Players::getTotalCount(PlayerState::Enemy, Protoss_Stargate) == 0)
            ...
            // ZealotArchon
                ...
                || (arrivesBy(1, Protoss_Archon, Time(8, 15)) && completesBy(1, UpgradeTypes::Leg_Enhancements, Time(8, 00))))
