...
namespace McRave::Combat::State {
    ...
    bool forceGlobalAttack(UnitInfo &unit)
    {
        ...
        const auto nearGrdToAir = [&]() {
            const auto closestDefense = Util::getClosestUnit(unit.getPosition(), PlayerState::Enemy, [&](auto &u) { return u->canAttackAir() && !u->isFlying(); });
            return closestDefense && closestDefense->getPosition().getDistance(target.getPosition()) < 256.0;
        };

        const auto engagingWithWorkers = [&]() {
            const auto closestCombatWorker = Util::getClosestUnit(target.getPosition(), PlayerState::Self,
                                                                  [&](auto &u) { return unit != *u && u->getType().isWorker() && u->getRole() == Role::Combat; });
            return closestCombatWorker && closestCombatWorker->getPosition().getDistance(target.getPosition()) < 96.0;
        };

        const auto freeAttacks = [&]() { return (!target.canAttackAir() && unit.isFlying()) || (!target.canAttackGround() && !unit.isFlying()); };
        ...
        return (atHome && unit.getSimState() == SimState::Win && !Players::ZvZ()) || (atHome && freeAttacks()) ||
               ...
               (!unit.isFlying() && Actions::overlapsActions(unit.unit(), unit.getPosition(), TechTypes::Dark_Swarm, PlayerState::Neutral, 96)) || (unit.isTargetedBySuicide() && !unit.isFlying()) ||
    ...
    void updateLocalState(UnitInfo &unit)
    {
        ...
        // If within local decision range, determine if Unit needs to attack or retreat
            ...
            unit.setLocalState(LocalState::Attack);
        else if (!unit.isLightAir() && Terrain::inTerritory(PlayerState::Self, unit.getPosition()) && (unit.getGlobalState() == GlobalState::Retreat || unit.getGoalType() == GoalType::Defend) &&
                 !unit.isLightAir())
            unit.setLocalState(LocalState::Hold);
        else if (!unit.isLightAir() && Terrain::inTerritory(PlayerState::Self, unit.getPosition()) && insideEngageRadius && unit.getSimState() != SimState::Win && !unit.isLightAir())
            unit.setLocalState(LocalState::Hold);
        ...
        else if (!unit.isLightAir() && insideEngageRadius && !insideRetreatRadius && unit.getSimState() != SimState::Win && !unit.isLightAir())
            unit.setLocalState(LocalState::Hold);
    }
