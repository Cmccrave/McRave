...
namespace McRave::BuildOrder::Protoss {
    ...
    void PvP_1GC_Robo()
    {
        ...
        // Buildings
        buildQueue[Protoss_Gateway]           = 1 + (vis(Protoss_Robotics_Facility) > 0);
    ...
    void PvP_1GC_4Gate()
    {
        ...
        if (Spy::getEnemyBuild() == P_2Gate) {
            ...
            buildQueue[Protoss_Gateway]          = 1 + (vis(Protoss_Pylon) >= 3) + (2 * (s >= 62));
            buildQueue[Protoss_Cybernetics_Core] = s >= 34;
        }
        else {
            buildQueue[Protoss_Shield_Battery]   = 0;
            buildQueue[Protoss_Gateway]          = 1 + (s >= 40) + (2 * (s >= 62));
            buildQueue[Protoss_Cybernetics_Core] = s >= 34;
        }
    ...
    void PvP_1GC_DT()
    {
        ...
        // Buildings
        buildQueue[Protoss_Nexus]            = com(Protoss_Dark_Templar) >= 2;
        buildQueue[Protoss_Gateway]          = 1 + (vis(Protoss_Templar_Archives) > 0);
        buildQueue[Protoss_Forge]            = s >= 70;
        buildQueue[Protoss_Citadel_of_Adun]  = vis(Protoss_Dragoon) > 0;
        buildQueue[Protoss_Templar_Archives] = com(Protoss_Citadel_of_Adun) > 0;

        auto firstZealotPump  = total(Protoss_Zealot) < zealotsNeeded_PvP();
        auto secondZealotPump = vis(Protoss_Photon_Cannon) >= 2 && s < 60;
