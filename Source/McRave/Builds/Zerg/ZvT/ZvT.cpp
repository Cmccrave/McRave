...
namespace McRave::BuildOrder::Zerg {
    ...
    int lingsNeeded_ZvT()
    {
        ...
        // Minimum lings
        auto minimumLings = (Util::getTime() > Time(4, 00) && Spy::getEnemyBuild() == T_RaxFact) ? 4 : 2;
    ...
    void ZvT_2HatchMuta()
    {
        ...
        unitPressure[Zerg_Mutalisk] = (Spy::getEnemyTransition() == T_2FactVulture && Util::getTime() < Time(10, 00)) ||
                                      (Players::getTotalCount(PlayerState::Enemy, Terran_Missile_Turret) == 0 && Util::getTime() < Time(10, 00));
        reserveLarva = 6;

        auto thirdHatch  = (Spy::enemyFastExpand() && vis(Zerg_Drone) >= 20);
        ...
        auto firstScourgePump = com(Zerg_Spire) > 0 && total(Zerg_Mutalisk) < 5 && Players::getVisibleCount(PlayerState::Enemy, Terran_Wraith) > vis(Zerg_Scourge) / 2;
        auto firstMutaPump    = com(Zerg_Spire) > 0 && !firstScourgePump && (total(Zerg_Mutalisk) < 12 || unitPressure[Zerg_Mutalisk]);
        ...
        // Gas
        gasLimit = gasMax();
        if (!Spy::enemyFastExpand() && !minerals(400)) {
    ...
    void ZvT_3HatchMuta()
    {
        ...
        if (!Spy::enemyFastExpand() && !minerals(400)) {
            auto dropGasLowDrone  = vis(Zerg_Drone) + vis(Zerg_Extractor) < 10;
            auto dropGasEarly     = (Spy::enemyProxy() || Spy::getEnemyOpener() == T_BBS) && Util::getTime() < Time(3, 30);
