...
namespace McRave::Command
{
    namespace {
        ...
        bool cast(UnitInfo& unit)
        {
            ...
            else if (unit.getType() == Terran_Science_Vessel && unit.unit()->getEnergy() >= TechTypes::Defensive_Matrix) {
            ...
            else if (unit.getType() == Protoss_Corsair && unit.unit()->getEnergy() >= TechTypes::Disruption_Web.energyCost() && Broodwar->self()->hasResearched(TechTypes::Disruption_Web)) {
        ...
        bool morph(UnitInfo& unit)
        {
            ...
            // Hydralisk - Lurker Morph
                ...
                if (vis(Zerg_Lurker) / BuildOrder::getCompositionPercentage(Zerg_Lurker) < vis(Zerg_Hydralisk) / BuildOrder::getCompositionPercentage(Zerg_Hydralisk) && canAffordMorph(Zerg_Lurker) && unit.getPosition().getDistance(unit.getSimPosition()) > 640.0) {
                    if (unit.unit()->getLastCommand().getType() != UnitCommandTypes::Morph)
            ...
            // Mutalisk - Devourer / Guardian Morph
                ...
                if (vis(Zerg_Devourer) / BuildOrder::getCompositionPercentage(Zerg_Devourer) < vis(Zerg_Hydralisk) / BuildOrder::getCompositionPercentage(Zerg_Mutalisk) && canAffordMorph(Zerg_Mutalisk) && unit.getPosition().getDistance(unit.getSimPosition()) > 640.0) {
                    if (unit.unit()->getLastCommand().getType() != UnitCommandTypes::Morph)
                        unit.unit()->morph(Zerg_Devourer);
                    return true;
                }
                else if (vis(Zerg_Guardian) / BuildOrder::getCompositionPercentage(Zerg_Guardian) < vis(Zerg_Hydralisk) / BuildOrder::getCompositionPercentage(Zerg_Mutalisk) && canAffordMorph(Zerg_Guardian) && unit.getPosition().getDistance(unit.getSimPosition()) > 640.0) {
                    if (unit.unit()->getLastCommand().getType() != UnitCommandTypes::Morph)
