...
bool UtilTrackerClass::shouldPullWorker(Unit unit)
{
	...
	else if ((Strategy().getEnemyBuild() == "Z5Pool" || Strategy().isRush()) && BuildOrder().isForgeExpand() && Units().getGlobalEnemyGroundStrength() > 0.00 && Broodwar->self()->completedUnitCount(UnitTypes::Protoss_Photon_Cannon) < 2 && Broodwar->self()->visibleUnitCount(UnitTypes::Protoss_Photon_Cannon) > 0) return true;
