...
namespace McRave
{
	...
	TilePosition BuildingTrackerClass::getBuildLocation(UnitType building)
	{
		...
		if (building.isResourceDepot()) return findExpoLocation();
		if (building.getRace() == Races::Zerg) return Broodwar->getBuildLocation(building, Terrain().getPlayerStartingTilePosition());
		...
		{
			Wall naturalWall = mapBWEB.getWall(mapBWEB.getNaturalArea());
	...
	bool BuildingTrackerClass::isBuildable(UnitType building, TilePosition buildTilePosition)
	{
		...
		if (usedTiles.find(buildTilePosition) != usedTiles.end()) return false; // TODO - Check all tiles not just the build position
		return true;
	}
	...
	TilePosition BuildingTrackerClass::findDefLocation(UnitType building, Position here)
	{
		...
		Station station = mapBWEB.getClosestStation(TilePosition(here));
	...
	TilePosition BuildingTrackerClass::findProdLocation(UnitType building, Position here)
	{
		...
		if (building.isRefinery())
		{
			...
			{
				ResourceInfo &gas = g.second;
				if (isQueueable(UnitTypes::Protoss_Assimilator, gas.getTilePosition()) && gas.getState() >= 2 && gas.getType() == UnitTypes::Resource_Vespene_Geyser && (gas.getPosition().getDistance(Terrain().getPlayerStartingPosition()) < best || best == 0.0))
				{
	...
	TilePosition BuildingTrackerClass::findExpoLocation()
	{
